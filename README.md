...
### 방법 1: Planner Agent → Strategy Agent 플로우 (권장)
1. 목표 입력 후 "전략 생성하기" 클릭
2. Planner Agent가 3가지 학습 스타일 제안
3. 원하는 스타일 선택 후 "주차별 상세 전략 받기" 클릭
4. Strategy Agent가 선택된 스타일에 최적화된 주차별 전략 생성

### 방법 2: Strategy Agent 직접 사용
1. "🎯 전략 설계 코치" 버튼 클릭
2. 목표, 기간, 과제 수, 키워드 직접 입력
3. Strategy Agent가 주차별 전략 생성

## 주요 기능

- **목표 입력**: 사용자가 학습 목표를 입력
- **Planner Agent (학습 프로세스)**: 3가지 학습 스타일별 전략 제안
- **Strategy Agent (전략 설계 코치)**: Planner Agent 결과를 바탕으로 주차별 상세 전략 생성
- **에이전트 연결**: Planner → Strategy Agent 간 데이터 전달
